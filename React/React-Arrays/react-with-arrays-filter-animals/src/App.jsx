...
export default function App() {
  ...
  const filteredAnimals = animals.filter((animal) => {
    return animal.habitat === selHabitat;
  });
    ...
    <>
      ...
      {habitats.map((habitat) => (
        <button
          key={habitat}
          className={
            selHabitat === habitat ? "button button--selected" : "button"
          }
          onClick={() => setSelHabitat(habitat)}
        >
          {habitat}
        </button>
      ))}
      {/* <h2>Animal result: {selHabitat}</h2> */}
      <ul>
        {filteredAnimals.map((filteredAnimal) => {
          return (
            <li key={filteredAnimal.name}>
              {filteredAnimal.name}
              {filteredAnimal.emoji}
            </li>
          );
        })}
      </ul>
