...
function maxOfTwoNumbers(num1, num2) {
  // TODO:  compare the numbers, return the biggers number
  // using a ternary operator or else if logic
  if (num1 > num2) {
    return num1;
  } else {
    return num2;
...
function findLongestWord(words) {
  // TODO:
  /*  
  1. find the length of the words.
  2. make a new array witjh just the longest word */

  const wordsSorted = words.sort((a, b) => b.length - a.length);
  const [longestWord, ...remainingWords] = wordsSorted;
  return longestWord;
}
...
function sumNumbers(numbers) {
  // TODO: make a let sum variable starting at null
  // itirate through the array
  // add the numbers to the sum variable
  // return the variable
  let sum = 0;
...
function averageWordLength(words) {
  ...
  itirate through the array and combine all string lengths
  divide it by the length of the array 
  */
  let stringLength = 0;
  for (let counter = 0; counter < words.length; counter++) {
    stringLength = stringLength + words[counter].length;
    console.log(
      "for counter" + counter + "the length is" + words[counter].length,
    );
    console.log("the string length is" + stringLength);
  }
  const averageString = stringLength / words.length;
  return averageString;
}
// console.log(averageWordLength(words2));

// Unique arrays -np return an array without duplicates
...
function doesWordExist(haystack, needle) {
  ...
  if (wordsFind.includes("haystack")) {
    return true;
  } else if (wordsFind.includes("needle")) {
    return true;
  } else {
    return false;
  }
}
console.log("does word exist: " + doesWordExist("haystack", "needle"));
// Count repetition
