...
const titlesWithDurationDestructured = recipes.map(
  ({ title, duration }) => `${title} (${duration}min)`,
);
console.log(titlesWithDurationDestructured); // ['Crepes (60min)', ...]

const timePerServing = recipes.map(
  (recipe) => recipe.duration / recipe.servings,
);
const timePerServingDestructured = recipes.map(
  ({ duration, servings }) => duration / servings,
); // [20, 10, 432, 12]
console.log(timePerServingDestructured);
...
const allTitlesDestructured = recipes.map(({ title }) => title).join(", ");
console.log("Destr" + allTitlesDestructured);
